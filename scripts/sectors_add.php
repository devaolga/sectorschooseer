...
$html_sectors = '<option value="1">Manufacturing</option>
...
$parentSpaceTree = [];
$parts = explode('<option ', $html_sectors);
array_shift($parts);
foreach ($parts as $part) {
    $sector = new Sector();
    $sector->id = (int) explode('"', $part)[1];
    $part = explode('</option>', $part)[0];
    if (strpos($part, '&nbsp;')) {
        $spaces_name = explode('&nbsp;', $part);
        $sector->name = array_pop($spaces_name);
        $sector->parentId = findParent(count($spaces_name));
    } else {
        $spaces_name = ["val"];
        $sector->name = explode('<;', explode('>', $part)[1])[0];
        $sector->parentId = 0;
    }
    $parentSpaceTree[$sector->id] = ['count' => count($spaces_name), 'parent_id' => $sector->parentId];
    echo (print_r($sector));
    if ($sector->save()) {
        echo 'OK';
    } else {
...
function findParent(int $spaces_count) : int
{
    global $parentSpaceTree;

    if ($spaces_count == 0) {
        return 0;
    }
    $lastSectorSpace = $parentSpaceTree[array_key_last($parentSpaceTree)]['count'];

    if ($spaces_count > $lastSectorSpace) {
        return array_key_last($parentSpaceTree);
    }

    $reversedTree = array_reverse($parentSpaceTree);
    foreach($reversedTree as $sector) {
        if ($sector['count'] == $spaces_count) {
            return $sector['parent_id'];
        }
    }
}